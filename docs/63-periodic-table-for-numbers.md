...
> The "atoms of counting" idea: primes are elements, composites are molecules, and the prime-factorization vector $s_n$ is their structural formula. A notebook that builds a Mendeleev-style grid for the integers.
>
> **Source:** `QUESTIONS ABOUT PRIMES/000_FACTORIZATION_MAN/PeriodicTableForNumbers.ipynb`.

## Intro

I don't use $p$ for prime factors because later on I will use p to repsesent the sequences of all primes themselves. I add this disclaimer because I don't want to cause confusion between the prime factors of a number and the sequence of all primes.

So please allow me to express the prime factorization of a number $n$ in terms of what in number th in number theory called big omega and little omega,but witha a different notation. Here I will use $M_n$ to represent big omega, which is the total number of prime factors of $n$ with multiplicity, and $m_n$ to represent little omega, which is the number of distinct prime factors of $n$. 
...
|----------|-----------|-------------|
| $s_n$ | $\sum_{k=1}^{M_n} e_k \cdot \omega^{d_k}$ | All prime factors with multiplicity |
| $\dot{s}_n$ | $\sum_{k=1}^{m_n} \omega^{d_k}$ | Distinct prime factors only |

where $\omega = e^{2\pi i / T_o}$, for $T_o \in \mathbb{Q}$, the $b_k$ are the prime factors of $n$ with multiplicity, the $d_k$ are the distinct prime factors, the $e_k$ are the exponents of each $d_k$, and $M_n$ is the total number of prime factors of $n$ with multiplicity, which can be expressed as:

$$m_n \space  \text{is the number of distinct prime factors of } n$$

And

$$M_n = \sum_{k=1}^{m_n} e_k$$
...
So, $s_n$ captures the full prime factorization of $n$ including multiplicities, while $\dot{s}_n$ captures only the distinct prime factors without considering their multiplicities.
